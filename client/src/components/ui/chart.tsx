...
// On fixe les génériques du tooltip (valeur = number, nom = string)
...
// -----------------------------------------------------------------------------
// ChartStyle (variables CSS par thème)
// -----------------------------------------------------------------------------
...
type CustomTooltipProps = TooltipContentProps<TooltipValue, TooltipName> &
  React.ComponentProps<"div"> & {
    ...
    labelFormatter?: (
      label: TooltipContentProps<TooltipValue, TooltipName>["label"],
      payload: TooltipContentProps<TooltipValue, TooltipName>["payload"]
    ) => React.ReactNode
  }
...
const ChartTooltipContent = React.forwardRef<HTMLDivElement, CustomTooltipProps>(
  (
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      const [item] = payload as Payload<number, string>[]
      ...
      >
        ...
        <div className="grid gap-1.5">
          {typedPayload.map((item, index) => {
            ...
            const reactKey = String(item.dataKey ?? item.name ?? index)
    ...
    return (
      ...
      >
        {payload.map((item, index) => {
          ...
          const reactKey = String(item.dataKey ?? item.value ?? index)
...
// -----------------------------------------------------------------------------
// Helper commun pour récupérer la config à partir d’un payload
// -----------------------------------------------------------------------------
